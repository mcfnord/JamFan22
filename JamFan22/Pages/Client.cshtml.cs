    ...
    public class ClientModel : PageModel
    {
        ...
        public string InfoHtml
        {
            get
            {
                ...
                string ipLon1 = "<code>139.144.151.196</code>";
                string ipLon2 = "<code>13.42.109.202</code>";
                string ipFfm = "<code>3.72.75.211</code>";
                ...
                switch (NationCode)
                {
                    case "CN": case "TW": case "HK": return
                        ...
                        $"<li>如要防止基于IP推断加入您服务器的音乐家的地理信息，请屏蔽来自 {ip2} 的<i>入站</i> UDP 流量。</li>" +
                        $"<li>如要将您的服务器从此列表中完全移除，还需屏蔽：<ul><li>{ipLon1}</li><li>{ipLon2}</li><li>{ipFfm}</li></ul></li></ul>" +
                    ...
                    case "TH": return
                        ...
                        $"<li>เพื่อป้องกันการอนุมานรายละเอียดทางภูมิศาสตร์ของนักดนตรีที่เข้าร่วมเซิร์ฟเวอร์ของคุณจาก IP ให้บล็อกการรับส่งข้อมูล UDP <i>ขาเข้า</i> จาก {ip2}</li>" +
                        $"<li>หากต้องการลบเซิร์ฟเวอร์ของคุณออกจากรายการนี้ทั้งหมด ให้บล็อกเพิ่มเติม:<ul><li>{ipLon1}</li><li>{ipLon2}</li><li>{ipFfm}</li></ul></li></ul>" +
                    ...
                    case "DE": return
                        ...
                        $"<li>Um zu verhindern, dass geografische Details über Musiker, die deinem Server beitreten, anhand der IP abgeleitet werden, blockiere <i>eingehenden</i> UDP-Verkehr von {ip2}.</li>" +
                        $"<li>Um deinen Server vollständig aus dieser Liste zu entfernen, blockiere außerdem:<ul><li>{ipLon1}</li><li>{ipLon2}</li><li>{ipFfm}</li></ul></li></ul>" +
                    ...
                    case "IT": return
                        ...
                        $"<li>Per impedire che i dettagli geografici sui musicisti che si uniscono al tuo server vengano dedotti dall'IP, blocca il traffico UDP <i>in entrata</i> da {ip2}.</li>" +
                        $"<li>Per rimuovere completamente il tuo server da questo elenco, blocca anche:<ul><li>{ipLon1}</li><li>{ipLon2}</li><li>{ipFfm}</li></ul></li></ul>" +
                    ...
                    case "ES": case "MX": case "AR": case "CO": case "CL": case "PE": case "VE": case "EC": case "GT": case "CU": case "BO": case "DO": case "HN": case "PY": case "SV": case "NI": case "CR": case "PA": case "UY": return
                        ...
                        $"<li>Para evitar que los detalles geográficos de los músicos que se unen a tu servidor se deduzcan a partir de la IP, bloquea el tráfico UDP <i>entrante</i> desde {ip2}.</li>" +
                        $"<li>Para eliminar completamente tu servidor de esta lista, bloquea también:<ul><li>{ipLon1}</li><li>{ipLon2}</li><li>{ipFfm}</li></ul></li></ul>" +
                    ...
                    case "NL": return
                        ...
                        $"<li>Om te voorkomen dat geografische details over muzikanten die op jouw server inloggen worden afgeleid op basis van IP, blokkeer <i>inkomend</i> UDP-verkeer van {ip2}.</li>" +
                        $"<li>Om jouw server volledig uit deze lijst te verwijderen, blokkeer ook:<ul><li>{ipLon1}</li><li>{ipLon2}</li><li>{ipFfm}</li></ul></li></ul>" +
                    ...
                    case "FR": case "BE": case "CH": return
                        ...
                        $"<li>Pour empêcher que les détails géographiques sur les musiciens qui rejoignent votre serveur soient déduits à partir de l'IP, bloquez le trafic UDP <i>entrant</i> depuis {ip2}.</li>" +
                        $"<li>Pour retirer complètement votre serveur de cette liste, bloquez également :<ul><li>{ipLon1}</li><li>{ipLon2}</li><li>{ipFfm}</li></ul></li></ul>" +
                    ...
                    default: return
                        ...
                        $"<li>To prevent geography details about musicians that join your server from being inferred based on IP, block <i>incoming</i> UDP traffic from {ip2}.</li>" +
                        $"<li>To remove your server from this list entirely, also block:<ul><li>{ipLon1}</li><li>{ipLon2}</li><li>{ipFfm}</li></ul></li></ul>" +
