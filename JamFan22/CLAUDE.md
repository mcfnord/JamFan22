...
- `GET /debug/welcome-preview` — **localhost only** — preview welcome for guid/nation/server; see Testing section in ops/CLAUDE.md
- `GET /debug/fleet-levels` — **localhost only** — current `m_fleetClientLevels` as JSON; per-server quiet flag + per-player name/level/audible
...
**`FleetIpAllowlist`** — strips port from `ip:port:rpc` lines in `fleet-server-ips.txt` for IP-only checks.
...
- **TCP connect timeout**: 5-second `CancellationTokenSource`.
