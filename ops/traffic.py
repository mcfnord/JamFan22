...
TIER1 = {'friend_visibility','click_musician','click_listen','click_more','nearby_toggle',
         ...
         'nearby_layout','grid_layout','hover_server','scroll_depth'}
...
ips = defaultdict(lambda: {'events': set(), 'ua': '', 'ref': False})
...
with open('/root/JamFan22/JamFan22/data/telemetry.log') as f:
    for line in f:
        ...
        ips[ip]['events'].add(payload.get('a', ''))
        if payload.get('ua'): ips[ip]['ua'] = payload['ua']
        if 'jamulus.live' in str(payload.get('ref', '')): ips[ip]['ref'] = True
...
print(f"Last {WINDOW_MINUTES} min — {len(ips)} unique IPs")
